...
def analysis(connection):
    ...
    fs_data = pred_d.get_pred_data(connection[index1]['segmentStart']['title'], connection[index1]['train']['name'],
                                   fromUnix(connection[index1]['departure'][time]))
    ls_data = pred_d.get_pred_data(connection[-index2]['segmentDestination']['title'],
                                   connection[-index2]['train']['name'],
                                   fromUnix(connection[-index2]['departure'][time]))
    if not fs_data:
    ...
    for i in range(index1, len(connection) - index2):
        ...
        if connection[i + 1]['train']['name'] != 'Fußweg':
            data1 = pred_d.get_pred_data(connection[i]['segmentDestination']['title'], connection[i]['train']['name'],
                                         fromUnix(connection[i]['arrival'][time]))
            data2 = pred_d.get_pred_data(connection[i + 1]['segmentStart']['title'], connection[i + 1]['train']['name'],
                                         fromUnix(connection[i + 1]['departure'][time]))
            transtime = ((fromUnix(connection[i + 1]['departure'][time]) - fromUnix(
                connection[i]['arrival'][time])).seconds // 60) % 60  # we just want minutes
            logger.debug(str(fromUnix(connection[i + 1]['departure'][time])) + " - " + str(
                fromUnix(connection[i]['arrival'][time])) + " = " + str(transtime))
            connection[i]['con_score'], connection[i]['adelay5'], connection[i + 1]['ddelay5'] = rfp.predict_con(data1,
                                                                                                                 data2,
                                                                                                                 transtime)
        else:
            data1 = pred_d.get_pred_data(connection[i]['segmentDestination']['title'], connection[i]['train']['name'],
                                         fromUnix(connection[i]['arrival'][time]))
            data2 = pred_d.get_pred_data(connection[i + 2]['segmentStart']['title'], connection[i + 1]['train']['name'],
                                         fromUnix(connection[i + 1]['departure'][time]))
            transtime = ((fromUnix(connection[i + 2]['departure'][time]) - fromUnix(
                connection[i]['arrival'][time])).seconds // 60) % 60  # we just want minutes
            logger.debug(str(fromUnix(connection[i + 2]['departure'][time])) + " - " + str(
                fromUnix(connection[i]['arrival'][time])) + " = " + str(transtime))
            connection[i]['con_score'], connection[i]['adelay5'], connection[i + 2]['ddelay5'] = rfp.predict_con(data1,
                                                                                                                 data2,
                                                                                                                 transtime)
        ...
        logger.debug("Score for connection[" + connection[i]['train']['name'] + ' to ' + connection[i + 1]['train'][
            'name'] + ' in ' + connection[i]['segmentDestination']['title'] + "] = " + str(connection[i]['con_score']))
        if total_score == 0:
...
def connect():
    ...
    # data = {
    #     "bhf": pd.read_csv(basepath + "/static_data/auto_complete_bhfs.csv", sep=",", index_col=False)["bhf"].tolist()}
    resp = jsonify(data)
    ...
    if request.form['key'] == current_app.config["DEPLOY_KEY"]:
        ...
        if git == "1":
            ...
            if "dev" not in request.form and not current_app.debug:
                ...
                reset = subprocess.run(["/usr/bin/git", '-C', basepath, 'reset', '--hard', 'HEAD^'],
                                       stdout=subprocess.PIPE).stdout.decode('utf-8')
            fetch = subprocess.run(['/usr/bin/git', '-C', basepath, 'fetch'], stdout=subprocess.PIPE).stdout.decode(
                'utf-8')
            merge = subprocess.run(
                ['/usr/bin/git', '-C', basepath, 'merge', '-s', 'recursive', '-X', 'theirs', '--no-commit'],
                stdout=subprocess.PIPE).stdout.decode('utf-8')
            logger.warning("git merge said: " + merge)
            git = subprocess.run(['/bin/bash', basepath + '/checkgit.sh'], stdout=subprocess.PIPE).stdout.decode(
                'utf-8')
            ...
            if git == "1":
                ...
                def on_close():
                    logger.warning(
                        subprocess.run(['/bin/bash', basepath + '/restart.sh'], stdout=subprocess.PIPE).stdout.decode(
                            'utf-8'))
                    return
...
def gitid():
    ...
    if request.form['key'] == current_app.config["DEPLOY_KEY"]:
        git = subprocess.run(["/usr/bin/git", '-C', basepath, 'rev-parse', '@'], stdout=subprocess.PIPE).stdout.decode(
            'utf-8').replace("\n", "")
