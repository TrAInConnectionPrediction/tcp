...
for i in range(1,21):
    for j in range(1,21):
        model = RandomForestClassifier(n_estimators=i*5, n_jobs=-1,max_depth=j*5, random_state=random_state)
        model.fit(X_train ,y_train)
        model_classified = model.predict(X_test)
        infos[str(i) + "+" + str(j)] = 1 - ((y_test != model_classified).sum() / X_test.shape[0])
    print(infos)
print("DONE \n\n\n\n\n\n")
print(infos)
