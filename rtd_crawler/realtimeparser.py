...
from database import sql_types, Change, PlanById, UnparsedChange, UnparsedPlan, Rtd, Session
...
parser.add_argument('--parse_continues', help='Check for unparsed data every 30 seconds and parse it')

parser.add_argument('--parse_all', help='Parse all raw data that is in the databse')
...
def parse_batch(hash_ids: List[int]):
...
def parse_chunk(chunk_limits: Tuple[int, int]):
    with Session() as session:
        hash_ids = PlanById.get_hash_ids_in_chunk_limits(session, chunk_limits)
    parse_batch(hash_ids)
